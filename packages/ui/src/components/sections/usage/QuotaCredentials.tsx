...
export const QuotaCredentials: React.FC<{ providerId: ProviderId; providerName: string }> = ({ providerId, providerName }) => {
  ...
  const field = (name: string, label: string, placeholder: string, type: React.HTMLInputTypeAttribute = 'password') => <label className="block typography-ui-label text-foreground">{label}<Input className="mt-1 h-7 font-mono text-xs" type={type} autoComplete="off" value={values[name] ?? ''} onChange={(event) => setValues((current) => ({ ...current, [name]: event.target.value }))} placeholder={type === 'password' ? status?.secretMasked ?? placeholder : placeholder} /></label>;
