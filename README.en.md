...
# <picture><source media="(prefers-color-scheme: dark)" srcset="docs/references/badges/openchamber-logo-dark.svg"><img src="docs/references/badges/openchamber-logo-light.svg" width="32" height="32" align="absmiddle" /></picture> OpenChamber LingXiFox

> [!IMPORTANT]
> **OpenChamber LingXiFox** is an independent downstream distribution of [OpenChamber](https://github.com/openchamber/openchamber).
> It tracks upstream while adding LingXiFox-specific integrations, workflows, and product changes. It is not an official OpenChamber release.
...
| **VS Code** | **Not currently supported.** The LingXiFox distribution does not yet provide its own VS Code extension |
...
Download the latest release from [GitHub Releases](https://github.com/LingXiFox/openchamber-lingxi/releases/latest). Desktop bundles the matching OpenCode CLI, so no separate OpenCode installation is required.
...
> [!NOTE]
> **The LingXiFox distribution does not currently support a VS Code extension.**
> The OpenChamber extension on the Visual Studio Marketplace is the official upstream version and does not include LingXiFox-specific changes.

### CLI — Web and PWA

> [!NOTE]
> **The LingXiFox distribution does not yet provide a separately published CLI/Web package.**
> The one-line installer below installs the official upstream `@openchamber/web` package and does not include LingXiFox-specific changes. For the LingXiFox distribution, use the source tree or desktop releases from this repository for now.

The upstream CLI/Web requires Node.js 22+ and uses your installed [OpenCode CLI](https://opencode.ai).
...
The original copyright notice and license terms from the upstream OpenChamber project remain unchanged. **OpenChamber LingXiFox** is maintained by [LingXiFox](https://github.com/LingXiFox).
